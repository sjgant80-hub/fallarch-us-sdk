...
// Extracted from fallarch-us/index.html · 139216 bytes of source logic
...
// Sovereign project management + US law engine for State Bar-regulated firms
...
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Architecture opinion signed and retained per State Bar 13.5',{id:a.id,projectId:a.projectId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallarch</strong> is a tool for State Bar-regulated US attorneys. It assists with project management, CDD, document generation, and State Bar Accounts Rules tracking. It is <strong>not regulatory submission or architecture opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <p class="lead">Sovereign project management + US law research for 1-10 person State Bar-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>attorney</option><option>partner</option><option>paraarchitecture</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function tabAdvice(m){
 ...
 return `<div class="card"><h3>Issue architecture opinion <span class="meta">State Bar 13.5 retention · 6yrs</span></h3>
...
async function askT3(){
 ...
 const sys='You are a US architecture research assistant for State Bar-regulated attorneys. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated architecture advice — research only. Always say "verify against primary sources".';
