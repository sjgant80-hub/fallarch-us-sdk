// fallarch-us SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from fallarch-us/index.html · 139135 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "fallarch-us" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Fallarch v1.0.0 · prime 743 · MIT
// Sovereign project management + UK law engine for SRA-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='fallarch-us';const VERSION='1.0.0';const PRIME=743;const STORE='fallarch-us';const CONFIG_V='fallarch-us@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','projects','advice','corpus','weaves','audit','settings'];
...
const T0_RULES=[
 ...
 {q:'CFA vs DBA?',a:'CFA (Conditional Fee Agreement): "no win, no fee" with success fee uplift (max 100% of base costs, capped at 25% of damages excl. future care in PI). Governed by Courts & Architecture Services Act 1990 s.58, CFA Order 2013. DBA (Damages-Based Agreement): payment is % of damages recovered (50% civil, 35% employment, 25% PI). DBA Regs 2013. CFA success fee recoverable from client only since LASPO 2012. Hybrid CFA/DBA unenforceable: Zuberi v Lexlaw [2021] EWCA Civ 16.'},
 ...
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order architecture representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Architecture Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
...
const CORPUS=[
 ...
 {id:'lsa2007',title:'Architecture Services Act 2007',area:'other',summary:'Modernised architecture services regulation; created Architecture Services Board, LeO, ABS structures; defined reserved architecture activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to architecture advice (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',projectTab:'overview',activeProjectId:null,
 firm:null,advisers:[],clients:[],projects:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.projects=(await idbAll('projects'))||[];
 ...
 if(meta){state.activeProjectId=meta.activeProjectId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.projectTab=meta.projectTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeProjectId:state.activeProjectId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,projectTab:state.projectTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const projectId=(payload&&payload.projectId)||state.activeProjectId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+projectId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,projectId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveProject(m,reason,noBc){m.updatedAt=Date.now();await idbPut('projects',m);if(!state.projects.find(x=>x.id===m.id))state.projects.push(m);else state.projects=state.projects.map(x=>x.id===m.id?m:x);await audit('project.updated',reason||'Project updated',{id:m.id,projectId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('project.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Architecture opinion signed and retained per SRA 13.5',{id:a.id,projectId:a.projectId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,projects:state.projects});
 ...
 for(const mt of(p.projects||[])){const local=state.projects.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveProject(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='project.created'||m.type==='project.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.projects.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveProject(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 for(const m of state.projects){
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',projectId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
...
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForProject:''}},
...
function newProjectRec(clientId){
 const seq=(state.firm&&state.firm.nextProjectRefSeq)||(state.projects.length+1);
 if(state.firm)state.firm.nextProjectRefSeq=seq+1;
 ...
 title:'New project',practiceArea:'civil-litigation',
...
function activeProject(){return state.projects.find(m=>m.id===state.activeProjectId)}
function projectClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 mt.demo=true;
 await saveProject(mt,'demo seed',true);
 state.activeProjectId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setProjectTab(id){state.projectTab=id;saveMeta();render()}
function selectProject(id){state.activeProjectId=id;state.active='project';state.projectTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activeProject();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no project selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const projects=state.projects.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(projectClient(m)?clientLabel(projectClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 ...
 sb.innerHTML=`
 <h4>Projects · ${state.projects.length}</h4>
 ...
 <button class="btn primary sm" onclick="newProject()">+ new project</button>
 </div>
 <div class="project-list">${projects.length?projects.map(m=>{
 const cl=projectClient(m);
 ...
 return `<div class="project-card ${state.activeProjectId===m.id?'active':''}" onclick="selectProject('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no projects yet · click + new project</div>'}</div>`;
}
function render(){
 ...
 else if(a==='projects')v.innerHTML=viewProjects();
 else if(a==='project')v.innerHTML=viewProject();
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallarch</strong> is a tool for SRA-regulated UK solicitors. It assists with project management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or architecture opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Fallarch</h1>
 <p class="lead">Sovereign project management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>paraarchitecture</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.projects.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.projects.filter(m=>m.status==='closed').length;
 const critical=state.projects.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.projects){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.projects.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no projects</div>'}
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectProject('${m.id}')">open →</button></div>`
...
function viewProjects(){
 const mm=state.projects.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Projects · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newProject()">+ new project</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=projectClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectProject('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleSolicitorId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no projects · click + new project</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewProject(){
 const m=activeProject();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No project selected</h3><p>Choose one from the sidebar, or <a onclick="newProject()">create new</a>.</p></div>';
 const cl=projectClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeProject('${m.id}')">${m.status==='closed'?'reopen':'close project'}</button>
 <button class="btn sm ox" onclick="deleteProject('${m.id}')">delete</button>
 </div>
 </div>
 <div class="project-tabs">${tabs.map(t=>`<button class="${state.projectTab===t[0]?'active':''}" onclick="setProjectTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.projectTab==='overview'?tabOverview(m,cl):''}
 ${state.projectTab==='retainer'?tabRetainer(m):''}
 ${state.projectTab==='conflict'?tabConflict(m,cl):''}
 ${state.projectTab==='timeline'?tabTimeline(m):''}
 ${state.projectTab==='advice'?tabAdvice(m):''}
 ${state.projectTab==='documents'?tabDocs(m):''}
 ${state.projectTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Project detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updProject('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updProject('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updProject('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updProject('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible solicitor</label><select onchange="updProject('responsibleSolicitorId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleSolicitorId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updProject('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updProject('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updProject('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updProject('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updProject('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updProject('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updProject('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','architecture-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updProject('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updProject('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updProject('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updProject('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updProject('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updProject('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updProject('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updProject('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local projects + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (project)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForProject)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this project</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updProject('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updProject('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updProject('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.projectId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue architecture opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>fallarch-uspaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>fallarch-uspractice</strong>. This view shows ledger ref IDs.</p>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic architecture patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeProjectId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeProjectId}','${w.id}')">apply to active project</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeProjectId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeProject().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / project.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Project</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.projects.find(x=>x.id===a.projectId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updProject(field,val){const m=activeProject();if(!m)return;m[field]=val;saveProject(m,'field '+field+' updated')}
async function newProject(){
 const m=newProjectRec();state.activeProjectId=m.id;
 await saveProject(m,'new project');await audit('project.created','New project created',{id:m.id,projectId:m.id,ref:m.ref});broadcast('project.created',m);
 state.active='project';state.projectTab='overview';saveMeta();render();
}
async function closeProject(id){const m=state.projects.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveProject(m,'project '+m.status);if(m.status==='closed')broadcast('project.closed',m);else broadcast('project.reopened',m);render()}
async function deleteProject(id){if(!confirm('Delete this project? (audit log preserved)'))return;await idbDel('projects',id);state.projects=state.projects.filter(m=>m.id!==id);if(state.activeProjectId===id)state.activeProjectId=null;await audit('project.deleted','Project deleted',{id});render()}
async function sendClientCare(id){const m=state.projects.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveProject(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.projects.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveProject(m,'event added');render()}
async function addFileRef(id){const m=state.projects.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveProject(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.projects.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveProject(m,'file ref removed');render()}
async function issueAdvice(projectId){
 const m=state.projects.find(x=>x.id===projectId);if(!m)return;
...
async function runConflictCheck(projectId){
 ...
 await saveProject(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— Fallarch T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeProject();const cl=m?projectClient(m):null;
 const ctx=m?`\nActive project: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK architecture research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated architecture advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Fallarch T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('fallarch-us-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,projects:state.projects,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('fallarch-us-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.projects.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('projects',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.projects=state.projects.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeProjectId&&!state.projects.find(m=>m.id===state.activeProjectId))state.activeProjectId=null;
 await audit('demo.purged','Demo data removed',{projects:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Fallarch booted',{version:VERSION,prime:PRIME});
})();
